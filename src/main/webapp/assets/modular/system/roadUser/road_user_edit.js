...
layui.use(['layer', 'form', 'admin', 'ax'], function () {
    ...
    var roadUserId=Feng.getUrlParam("roadUserId");;
    ...
    $.ajax({
        type: "get",
        url: Feng.ctxPath + '/sys/road_user/selectOneInspector?inspectorId='+roadUserId,
        dataType: 'json',
    ...
    form.on('submit(btnSubmit)', function (data) {
         ...
         var ajax1 = new $ax(Feng.ctxPath + '/sys/road_user/judgeAccount2?account='+account+'&inspectorId='+roadUserId, function (data) {
            if(data>0)
            {
                flagAccount=0;
                Feng.error("账户已存在");
         ...
         if(organizationId==-1){
          ...
          var ajax = new $ax(Feng.ctxPath + "/sys/road_user/updateInspector?roadUserName="+roadUserName+"&gender="+gender+"&age="+age+"&telephone="+telephone+"&organizationId="+organizationId+"&account="+account+"&inspectorId="+roadUserId, function (data) {
                      Feng.success("修改成功！");
