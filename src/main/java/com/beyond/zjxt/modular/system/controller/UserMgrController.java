...
import com.beyond.zjxt.core.common.constant.state.ManagerStatus;
...
public class UserMgrController extends BaseController {
    ...
    public Object getUserInfo(@RequestParam Long userId) {
        ...
        hashMap.put("sexName",ConstantFactory.me().getSexName(user.getSex()));
    ...
    @ResponseBody
    public Map<String, Object> modifyPwd(@RequestParam("useId") Long userId,@RequestParam("oldNum")String oldNum,@RequestParam("newNum")String newNum) {
        Map<String, Object> map = new HashMap<>();
        int i = this.userService.modifyPwd(userId,oldNum,newNum);
        if (ToolUtil.isOneEmpty(userId,oldNum,newNum)) {
            map.put("statusCode", 201);
            map.put("msg", "旧密码或新密码为空");
        }
        if (i == 1) {
    ...
    @ResponseBody
    public Long getRole(){
        Long  roleId  = ShiroKit.getUser().getRoleList().get(0);
        return roleId;
    }
}
