...
import com.beyond.zjxt.core.common.constant.Const;
import com.beyond.zjxt.core.common.constant.cache.Cache;
...
import com.beyond.zjxt.core.util.CacheUtil;
import com.beyond.zjxt.modular.system.entity.Relation;
import com.beyond.zjxt.modular.system.entity.Role;
import com.beyond.zjxt.modular.system.mapper.RelationMapper;
import com.beyond.zjxt.modular.system.mapper.RoleMapper;
import com.beyond.zjxt.modular.system.model.RoleDto;
...
public class RoleService extends ServiceImpl<RoleMapper, Role> {
    ...
    public List<ZTreeNode> roleTreeListUseByDept(Long[] roleId) {

        List<ZTreeNode> zTreeNodeList = this.baseMapper.roleTreeListUseByDept(roleId);
        boolean flag=false;
        int pointer=-1;
        /**暂时专门对养护站的特殊三级数据结构进行一个特殊的安排*/
        for (int i = 0; i < zTreeNodeList.size() ; i++) {

            if(zTreeNodeList.get(i).getId().equals(10L)||zTreeNodeList.get(i).getId().equals(1201025605710802946L)){
                flag=true;
                pointer=i;
            }
        }
        ...
        if (flag==true){
            Long[] longs= new Long[] {10L};
            List<ZTreeNode> zTreeNodeList2 = this.baseMapper.roleTreeListUseByDept(longs);
            for (ZTreeNode item1:zTreeNodeList2) {
                if( item1.getId().equals(10L)){
                    item1.setChecked(false);
                }

            }
            for (ZTreeNode item1:zTreeNodeList2) {
                zTreeNodeList.add(item1);
            }
            zTreeNodeList.remove(pointer);
        }
